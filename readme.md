...
## How does it work?
`spack.x [args]` consists of a slightly hacked version of the AppImage runtime concatenated
with a big squashfs file which includes `bzip2`, `clingo`, `curl`, `git`,
`gmake`, `gzip`, `openssl`, `patch`, `python`, `tar`, `unzip`, `xz`, `zstd` and
their dependencies.

When you run `./spack.x` it will use fusermount (through libfuse) to mount
